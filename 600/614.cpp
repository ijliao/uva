...
bool ispass(vector < vector < int > > &wall, POS p, int dir) {
	switch (dir) {
		case W :
			if (p.Y > 1)
...
void output(vector < vector < int > > &maze,
...
bool travel(vector < vector < int > > &maze,
			vector < vector < int > > &wall,
			POS cur, int count)
{
...
int main(void) {
	...
	while (true) {
		...
		vector < vector < int > >	wall(height, vector < int > (width));
